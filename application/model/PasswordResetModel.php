...
class PasswordResetModel
{
    ...
    public static function requestPasswordReset($user_email, $captcha)
    {
        ...
        // check if that user_email exists
        ...
        $token_set = self::setPasswordResetDatabaseToken($result->user_email, $user_password_reset_hash, $temporary_timestamp);		
        if (!$token_set) {
            return false;
        }
        ...
        $mail_sent = self::sendPasswordResetMail($user_password_reset_hash, $result->user_email);		
        if ($mail_sent) {
            return true;
        }
    ...
    public static function setPasswordResetDatabaseToken($user_email, $user_password_reset_hash, $temporary_timestamp)	
    {
...
*/		
        ...
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_reset_hash' => $user_password_reset_hash, ':user_email' => $user_email,
            ':user_password_reset_timestamp' => $temporary_timestamp, ':provider_type' => 'DEFAULT'
        ));		
    ...
    public static function sendPasswordResetMail($user_password_reset_hash, $user_email)	
    {
        // create email body
/*
...
*/				
        $body = Config::get('EMAIL_PASSWORD_RESET_CONTENT') . ' ' . Config::get('URL') .
                Config::get('EMAIL_PASSWORD_RESET_URL') . '/' . urlencode($user_email) . '/' . urlencode($user_password_reset_hash);				
    ...
    public static function verifyPasswordReset($user_email, $verification_code)	
    {
        $database = DatabaseFactory::getFactory()->getConnection();
...
*/		
        ...
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_reset_hash' => $verification_code, ':user_email' => $user_email,
            ':user_provider_type' => 'DEFAULT'
        ));		
    ...
    public static function saveNewUserPassword($user_email, $user_password_hash, $user_password_reset_hash)	
    {
        $database = DatabaseFactory::getFactory()->getConnection();
...
*/		
        ...
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_hash' => $user_password_hash, ':user_email' => $user_email,
            ':user_password_reset_hash' => $user_password_reset_hash, ':user_provider_type' => 'DEFAULT'
        ));		
    ...
    public static function setNewPassword($user_email, $user_password_reset_hash, $user_password_new, $user_password_repeat)	
    {
...
*/		
        if (!self::validateResetPassword($user_email, $user_password_reset_hash, $user_password_new, $user_password_repeat)) {
            return false;
        }		
		...
		if (self::saveNewUserPassword($user_email, $user_password_hash, $user_password_reset_hash)) {			
    ...
    public static function validateResetPassword($user_email, $user_password_reset_hash, $user_password_new, $user_password_repeat)	
    {
        if (empty($user_email)) {			
    ...
    public static function saveChangedPassword($user_email, $user_password_hash)	
    {
        $database = DatabaseFactory::getFactory()->getConnection();
    ...
    public static function changePassword($user_email, $user_password_current, $user_password_new, $user_password_repeat)	
    {
        // validate the passwords
        if (!self::validatePasswordChange($user_email, $user_password_current, $user_password_new, $user_password_repeat)) {			
            return false;
        }
        ...
        if (self::saveChangedPassword($user_email, $user_password_hash)) {			
            Session::add('feedback_positive', Text::get('FEEDBACK_PASSWORD_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_PASSWORD_CHANGE_FAILED'));
    ...
    public static function validatePasswordChange($user_email, $user_password_current, $user_password_new, $user_password_repeat)	
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "SELECT user_password_hash, user_failed_logins FROM users WHERE user_email = :user_email LIMIT 1;";		
        $query = $database->prepare($sql);
