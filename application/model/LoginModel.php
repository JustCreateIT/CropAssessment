...
class LoginModel
{
    ...
    public static function login($user_email, $user_password, $set_remember_me_cookie = null)	
    {
...
*/
        ...
        self::setSuccessfulLoginIntoSession(
            $result->user_id, $result->user_first_name, $result->user_last_name, $result->user_email, $result->user_phone_number, $result->user_account_type
        );		
    ...
    private static function validateAndGetUser($user_email, $user_password)	
    {
        ...
        if (!password_verify($user_password, $result->user_password_hash)) {
            ...
            self::incrementFailedLoginCounterOfUser($result->user_email);			
            Session::add('feedback_negative', Text::get('FEEDBACK_USERNAME_OR_PASSWORD_WRONG'));
            return false;
        }
    ...
    public static function loginWithCookie($cookie)
    {
        ...
        if ($result) {
            ...
            // successfully logged in, so we write all necessary data into the session and set "user_logged_in" to true
            self::setSuccessfulLoginIntoSession($result->user_id, $result->user_first_name, $result->user_last_name, $result->user_email,  $result->user_phone_number, $result->user_account_type);			
	...
	public static function setSuccessfulLoginIntoSession($user_id, $user_first_name, $user_last_name, $user_email, $user_phone_number,$user_account_type)
    {
        Session::init();
        ...
        Session::set('user_first_name', $user_first_name);	
        Session::set('user_last_name', $user_last_name);	
    ...
    public static function incrementFailedLoginCounterOfUser($user_email)	
    {
...
*/
    ...
    public static function saveTimestampOfLoginOfUser($user_email)	
    {
        $database = DatabaseFactory::getFactory()->getConnection();
/*
...
*/		
        ...
        $sth = $database->prepare($sql);
        $sth->execute(array(':user_email' => $user_email, ':user_last_login_timestamp' => time()));
    }
