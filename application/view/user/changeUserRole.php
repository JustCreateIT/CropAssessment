    ...
    <div class="box">
	    ...
	    <form action="<?php echo Config::get('URL'); ?>user/changeUserRole_action" method="post">
            <?php if (Session::get('user_account_type') == 1) { ?>
                <input type="submit" name="user_account_upgrade" value="Upgrade my account (to Standard User)" />
	        <?php } else if (Session::get('user_account_type') == 5) { ?>
	            <input type="submit" name="user_account_downgrade" value="Downgrade my account (to Public User)" />
	        <?php } ?>
	    </form>
    </div>
</div>
