...
class Auth
{
    ...
    public static function checkAuthentication()
    {
        ...
        if (!Session::userIsLoggedIn()) {
			...
			// remove redirection back to previous page at this stage
			// To-do - investigate when time suits
     ...
     * user role type 88 (currently there's only type 1 (public user), type 5 (standard user), type 9 (owner user),
	 * and type 88 (administrator).
     * If user is not administrator, then redirect to login page and the application is hard-stopped via exit().
     * Using this method only makes sense in controllers that should only be used by administrators.
     */
    public static function checkAdminAuthentication()
    {
        ...
        // if user is not logged in or is not an admin (= not role type 88)
        if (!Session::userIsLoggedIn() || Session::get('user_account_type') != 88) {	
