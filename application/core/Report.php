 ...
 */class Report
{
    ...
    public function HTMLReport_styles(){
        ...
        $html = '<style type="text/css">		
				...
				.report-table-growth td.content { 
					font-size: 1.5em; text-transform: capitalize; padding: 5px; color: #666; border: 1px solid #ccc; margin: 2px; width: 49%
					}				
    ...
    public function HTMLReport_header(){
        ...
        $html = '<!-- logo -->
				<div class="logo"></div>
				<!-- report title -->
				<div class="title"><h1>'.$this->report_name.' Assessment Report<h1></div>';
        return $html; 
    }
	...
	public function HTMLReport_data(){		
		$html = self::getReportDataByAssessmentType();
		return $html; 
	}	
	
	
	public function HTMLReport_table(){		
		$html = self::getReportSummaryByAssessmentType();
		return $html; 
	}
    ...
    */
	...
	private function getReportSummaryByAssessmentType(){
		//print_r($this->report_name);
		switch ($this->report_name){
			case 'Emergence':
				$html = self::getHTMLEmergenceAssessment();
				break;
			case 'Threeleaf':				
			case 'Fiveleaf':				
			case 'Bulbing':
				$html = self::getHTMLThreeFiveBulbingAssessment();
				break;
			case 'Harvest':
				$html = self::getHTMLHarvestAssessment();
				break;
			default:
	...
	*/
	...
	private function getHTMLSampleDataEmergence(){
		...
		foreach ($data as $zone) {
			...
			foreach ($zone as $key=>$sample_data) {
				$html .= '<tr><td>'.$sample_data->zone_name.'</td>
					<td>'.$key.'</td>
			...
			// summed averages row
			$html .= '<tr class="report-footer"><td style="text-align: right;" colspan="3">Zone Average</td>
	...
	private function getHTMLSampleDataHarvest(){
		...
		*/		
		...
		foreach ($data as $zone) {
			...
			$i = 1;
			foreach ($zone as $sample_data) {
				$html .= '<tr><td style="text-align: center">'.$sample_data->zone_name.'</td>
	...
	private function getHTMLEmergenceAssessment(){
		...
		$html = '<div>
			...
			$html .= '</tbody><tfoot class="report-footer"><tr>
					<td>&nbsp;</td>
					<td style="text-align: center; font-weight: bold">'.$sum_area.' ha</td>
					<td style="text-align: center; font-weight: bold">'.$sum_target_population.'</td>
					<td style="text-align: center; font-weight: bold">'.round(($sum_target_population/10000)/$sum_area).'</td>
					<td style="text-align: center; font-weight: bold">'.$sum_estimate_population.'</td>
					<td style="text-align: center; font-weight: bold">'.round(($sum_estimate_population/10000)/$sum_area).'</td>
					<td style="text-align: center; font-weight: bold">'.$cropZoneDifference.'%</td>
					<td>&nbsp;</td>';
	...
	private function getHTMLThreeFiveBulbingAssessment(){
		...
		*/
		...
		$html .= '<tfoot class="report-footer"><tr>
					<td colspan="4" style="text-align: right; font-weight: bold">Area Weighted Estimate: Total Crop Yield (tonnes)</td>
					<td colspan="4" style="text-align: center; font-weight: bold">';
		switch ($this->growth_stage_id){
			case 2:				
				$html .= $report_row->weighted_sum_three_leaf_yield.'<br>'.$report_row->three_leaf_tonnes_hectare;
				break;
			case 3:
				$html .= $report_row->weighted_sum_five_leaf_yield.'<br>'.$report_row->five_leaf_tonnes_hectare;
				break;
			case 4:
				$html .= $report_row->weighted_sum_bulbing_yield.'<br>'.$report_row->bulbing_tonnes_hectare;
				break;				
			default:
			// To-do - trap error
		}
	...
	private function getHTMLHarvestAssessment(){
		...
		$html .=  		'</tbody>
						...
						{
		$html .=  			'<tr>
								<td colspan="3" style="text-align: right; font-weight: bold">Area Weighted Estimate: Total Crop Yield (tonnes)</td>								
								<td style="text-align: center; font-weight: bold">'.$report_row->weighted_sum_three_leaf_yield.'<br>'.$report_row->three_leaf_tonnes_hectare.'</td>
								<td style="text-align: center; font-weight: bold">'.$report_row->weighted_sum_five_leaf_yield.'<br>'.$report_row->five_leaf_tonnes_hectare.'</td>
								<td style="text-align: center; font-weight: bold">'.$report_row->weighted_sum_bulbing_yield.'<br>'.$report_row->bulbing_tonnes_hectare.'</td>
								<td style="text-align: center; font-weight: bold">'.$report_row->weighted_sum_harvest_yield.'<br>'.$report_row->harvest_tonnes_hectare.'</td>
							</tr>';
						} 
		$html .=  '</tfoot>							
				</table>';
